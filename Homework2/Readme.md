...
- The raw results, or outputs, for the random number engines are stored in an Excel file located in the Homework 2 folder: HW2_Part1_StepA.xlsx (see 'raw results' sheet).
- Descriptive statistics for each random number engine are provided in the table below.
...
- Random number engine (RNE) ouput included aggregated data, specifically the frequency count of each value 0 through 100 generated by each RNE. This frequency data was used to construct five histograms:
...
- A Spearman's correlation was run to assess the relationship between the RNE results. There was a weak correlation between each of the RNE outputs. Somewhat suprisiningly, the weak negative correlation between the Ranlux results and the Default Random results was statistically significant (*r* = -.008, *p* = .008).
...
- Tested the distribution of the RNE outputs using a one-sample Kolmogorov-Smirnoff test with normal and uniform distributions; see tables below. Looking at the last row of the normal distribution table, we can see that the p-value is significant and that the RNE output does not have an approximate normal distribution (Lee, Famoye, Shelden, & Brown, n.d.). Conversely, none of the p-values in the uniform distribution table reached significance so we **can not** say that the data **does not** have an approximate uniform distribution.

| Testing for Distribution |
| ------------- |
| ![](images/onesampleks.png?raw=true)  |

- The data is uniform. We can visually assess the uniformity of the data by looking at the histogram graphs above. Nearly all of the histogram bars representing a subset within the set [0, 100] are equal in height (i.e., frequency). If the subset ranges are increased or decreased (e.g., +10 or -5 , respectively) the histogram bars remain relatively equal in height, excluding the first subset containing the smaller values. Looking at the histrograms we can also note that there seems to be a small bias in each random number engine for the first ten values 0 through 10. This bias might be attributable to the seed used for the random number engines.
