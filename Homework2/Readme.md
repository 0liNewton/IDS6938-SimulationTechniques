...
- Selected random number engines: Mersenne Twister, Knuth-B, Minimal Standard, Ranlux, and Sobol
- Raw output of the engines is stored in file HW2_Part1_StepA.xlsx ('raw results' sheet) in the Homework 2 folder
- Descriptive statistics for five random number engine are provided in the table below

|  | Mersenne  | Knuth | Min. St. | Ranlux | Sobol |
| ------------- | ------------- | ------------- | ------------- | ------------- | ------------- |
|*Mean*  | 49.98 | 49.81  | 50.06  | 50.08| 50  |
|*St. Dev.*  | 28.85 | 28.82 | 28.87 | 28.85 | 28.87 |


- Random number engine ouput included aggregated data, specifically frequency count of each value 0 through 100 generated by the five engines
- This frequency data was used to construct histograms:
...
- Spearman's correlation was run to assess the relationship between the engine results
- There was a weak correlation between each of the engine outputs
- Accidentally included output from a sixth random number engine, default random engine, in my analyzes
- Found that the weak negative correlation between Ranlux and Default was statistically significant (*r* = -.008, *p* = .008)

| Spearman's |
| ------------- |
| ![](images/spearmanrank.png?raw=true)  |

- Tested the distribution of the RNE outputs using a one-sample Kolmogorov-Smirnoff test with normal and uniform distributions (see tables below)
- Looking at the last row of the normal distribution table, we can see that the p-value is significant - the RNE output does not have an approximate normal distribution (Lee, Famoye, Shelden, & Brown, n.d.).
- Conversely, none of the p-values in the uniform distribution table reached significance so we **can not** say that the data **does not** have an approximate uniform distribution.
...
*Nearly all of the histogram bars representing a subset within the set [0, 100] are equal in height (i.e., frequency). If the subset ranges are increased or decreased (e.g., +10 or -5 , respectively) the resulting histograms are nearly indistinguishable from eachother and the histogram bars themselves remain relatively equal in height. The exception in all cases is the first bar of the histogram, and in tha case of small values on the x-axis (e.g., 1) also the last bar of the histogram excluding the first subset containing the smaller values. This can  lead us to note that there seems to be a bias for the first ten values, 0 through 100, in all five engines. This bias might be attributable to the seed used for the engines.*

- Can statistically assess the uniformity of the data by looking at the
