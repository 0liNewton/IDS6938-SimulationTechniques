...
> - The raw results, or outputs, for the random numer engines are stored in an Excel file located in the Homework 2 folder: HW2_Part1_StepA.xlsx (see 'raw results' sheet).
...
> - The data is uniform. We can visually assess the uniformity of the data by looking at the histogram graphs above. Nearly all of the histogram bars representing a subset within the set [0, 100] are equal in height (i.e., frequency). If the subset ranges are increased or decreased (e.g., +10 or -5 , respectively) the histogram bars remain equal in height, excluding the first subset containing the smaller values.
